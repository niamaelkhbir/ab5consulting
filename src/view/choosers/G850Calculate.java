...
public class G850Calculate extends SpaceObjectChooser
{

    public GUI gui;

    public G850Calculate(GUI gui)
    {
        this.gui = gui;
    ...
    public void activate(SpaceObject leftObject, SpaceObject rightObject)
    {
        try
        {
            double res = Calculations.G_850((Node)leftObject,(Sensor)rightObject);
            gui.notifyObservers("Sensor "+rightObject.getLabel()+" detects a signal from node "+leftObject.getLabel()+" with a gain of : "+res+" (Recommendation 850)");
            return;
        }
        catch (IOException e)
        {
